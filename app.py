...
with st.sidebar:
    ...
    st.markdown("Supports your file with Total Quantity Available, Average Price, Invested Amount, LTP, Current Value")
    uploaded_file = st.file_uploader("Upload Excel/CSV", type=["xlsx","xls","csv"], help="Your holdings-04.09.2026-for_Analysis.xlsx", key="holdings_upload")
    portfolio_stocks = []
    upload_debug = st.empty()
    
    if uploaded_file is not None:
        try:
            import io
            file_bytes = uploaded_file.getvalue()
            upload_debug.info(f"📄 Uploaded: {uploaded_file.name} ({len(file_bytes)} bytes)")
            buf = io.BytesIO(file_bytes)
            ext = uploaded_file.name.split('.')[-1].lower()
            
            # Try to read with pandas first to show debug
            try:
                debug_df = pd.read_excel(io.BytesIO(file_bytes), sheet_name=0)
                upload_debug.info(f"📊 Excel read: {debug_df.shape[0]} rows, {debug_df.shape[1]} cols | Columns: {', '.join(list(debug_df.columns)[:4])}...")
            except Exception as e:
                upload_debug.error(f"Excel read failed: {e}")
            
            if ext=='csv':
                parsed = parser.parse_csv(io.BytesIO(file_bytes))
            else:
                parsed = parser.parse_excel(io.BytesIO(file_bytes))
            
            if parsed and len(parsed)>0:
                total_inv = sum([x.get('invested_file',0) or 0 for x in parsed])
                st.success(f"✅ FIXED: {len(parsed)} stocks parsed! Total Quantity Available used. Invested: \u20b9{total_inv:,.0f}")
                st.caption(f"Example: {parsed[0]['ticker']} Qty {parsed[0]['quantity']:.0f} (Total Available) Avg \u20b9{parsed[0]['avg_price']:.2f}")
                portfolio_stocks = parsed
            else:
                st.error(f"❌ Parser returned 0 stocks. Check file format. Expected columns: Symbol, Total Quantity Available, Average Price")
                # Show what parser detected
                try:
                    df_check = pd.read_excel(io.BytesIO(file_bytes), sheet_name=0)
                    st.write("Columns found:", list(df_check.columns))
                    st.write("First row:", df_check.iloc[0].to_dict())
                except Exception as e:
    ...
    # Only load portfolio.json if NO upload attempted
    if not portfolio_stocks and uploaded_file is None:
        try:
            with open("portfolio.json") as f:
                data=json.load(f)
                portfolio_stocks=data.get('stocks',[])
                if portfolio_stocks:
                    st.info(f"📁 Sample portfolio.json: {len(portfolio_stocks)} stocks (from your Excel sample). Upload your full Excel for all 52.")
        except:
            portfolio_stocks=[]
            st.warning("No portfolio loaded. Upload your Excel file.")
    elif not portfolio_stocks and uploaded_file is not None:
        st.warning("⚠️ Uploaded file parsed 0 stocks, not using sample. Please check file format.")
    ...
    use_file_ltp = st.checkbox("Use File LTP (not Live)", value=True, help="Uses Last Traded Price from Excel - matches Excel P&L exactly. Uncheck for Live NSE price.")
    
    st.divider()
    st.header("⚙️ Actions")
    calc_pnl = st.button("💹 Calculate FIXED P&L", type="primary")
    calc_sip = st.button("📅 SIP + XIRR")
    calc_div = st.button("💰 Dividend")
    calc_cg = st.button("📊 STCG/LTCG")
    gen_reports = st.button(f"📄 Reports ({len(portfolio_stocks)} stocks)")

    # Show current loaded portfolio summary in sidebar
    if portfolio_stocks:
        st.divider()
        st.caption(f"Loaded: {len(portfolio_stocks)} stocks | Total Invested File: \u20b9{sum([x.get('invested_file',0) or 0 for x in portfolio_stocks]):,.0f}")
